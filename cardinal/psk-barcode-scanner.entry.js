...
const PskBarcodeScanner = class {
    ...
    render() {
        ...
        return [
            h("script", { async: true, src: `${cardinal}/libs/zxing.js` }),
            h("div", { title: this.title, style: style.barcodeWrapper }, this.isCameraAvailable && !this.isScanDone
                ? (h("div", { id: "scanner-container", style: style.videoWrapper }, h("input", { type: "file", accept: "video/*", capture: "camera", style: style.input }), h("video", { id: "video", muted: true, autoplay: true, playsinline: true, style: style.video }), h("button", { onClick: _ => this.switchCamera(), style: style.button }, "Change camera")))
                : h("div", null, this.status))
        ];
    }
